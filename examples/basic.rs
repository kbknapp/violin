...
fn main() {
    ...
    let die = rand::distributions::Uniform::from(1..5000); // synthetic latenies will range from 1 to 5000 ms
    let rtts: Vec<u64> = vec![0u64; 2 * 1000] // 1000 synthetic latencies for each node
        .iter_mut()
        .map(|_rtt| die.sample(&mut rng))
        .collect();

    for lats in rtts.chunks(2) {
        node.update(lats[0], peer.coordinate(), 25);
        peer.update(lats[1], node.coordinate(), 25);
    }
